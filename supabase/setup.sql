...
insert into public.specimens (no, name, year, tagline, blurb, reason, cause, sort_order, confidential) values ('FD-11', 'Well Travelled', 2026, 'a private map of every country a friend group has cooked from', 'One entry per country: host, date, dishes, attendees, photos, notes. A full-bleed zoomable world map, a shared host-and-attendee roster, "suggest a country" and "draw a mystery country" for when nobody''s volunteering, and a generative passport stamp per entry so the collection actually looks like something. Comments, promotions from suggestion to entry, the works.', 'Live at welltravelled.itshypothetical.com. Gated behind a shared password, because it was built for one specific group of friends, not the internet at large.', 'NOT EXTINCT, JUST NOT FOR YOU', 10, true) on conflict (no) do nothing;
